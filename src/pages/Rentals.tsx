  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { label: 'Locações Ativas', value: '28', color: 'bg-emerald-100 text-emerald-800', icon: 'sync' },
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
              ...
              {mockInvoices.map((rental, index) => (
                ...
                >
                  <td className="px-6 py-4">
                    <div className="font-bold text-slate-900">{rental.client_name}</div>
                    <div className="text-[11px] text-slate-500 uppercase font-medium">{rental.work_site}</div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-slate-600">
                    <div className="flex items-center gap-1">
                      ...
                      {new Date(rental.billing_period_start).toLocaleDateString()} - {new Date(rental.billing_period_end).toLocaleDateString()}
                    </div>
                  </td>
                  <td className="px-6 py-4 text-right font-bold text-emerald-900">
                    R$ {rental.total_value.toLocaleString()}
                  </td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold ${
                      rental.billing_status === 'Paga' 
                        ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' 
                        : 'bg-amber-100 text-amber-800 border border-amber-200'
                    }`}>
                      <span className="material-symbols-outlined text-[14px]">
                        {rental.billing_status === 'Paga' ? 'check_circle' : 'schedule'}
                      </span>
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/20 flex justify-between items-center text-xs text-slate-500 font-medium">
          <span>Mostrando 2 de 28 locações</span>
