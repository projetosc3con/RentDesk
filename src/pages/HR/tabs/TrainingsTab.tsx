        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="p-4 space-y-3">
            {[
              ...
              <div key={t.name} className="p-4 rounded-2xl border border-slate-100 hover:border-emerald-200 hover:bg-emerald-50 transition-all group cursor-pointer">
                <p className="text-xs font-bold text-slate-900">{t.name}</p>
                <span className="text-[10px] font-bold text-slate-400 uppercase group-hover:text-emerald-600">{t.cat}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="lg:col-span-3 space-y-6">
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-8 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h3 className="font-bold text-slate-900 text-lg">Histórico de Capacitações</h3>
              <p className="text-sm text-slate-500">Registro de todos os treinamentos concluídos pela equipe.</p>
            </div>
            <button 
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  ...
                  <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Conclusão</th>
                  <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Carga Horária</th>
                  <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Certificado</th>
                </tr>
              </thead>
                ...
                {mockTrainings.map((t) => (
                  <tr key={t.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-8 py-6">
                      <p className="text-sm font-bold text-slate-900">{t.employee}</p>
                    </td>
                    <td className="px-8 py-6">
                      <p className="text-sm text-slate-700 font-medium">{t.training}</p>
                    </td>
                    <td className="px-8 py-6">
                      <p className="text-xs text-slate-500 font-bold">{t.date}</p>
                    </td>
                    <td className="px-8 py-6">
                      <span className="text-[10px] font-black bg-slate-100 px-2 py-1 rounded-md text-slate-600">{t.workload}</span>
                    </td>
                    <td className="px-8 py-6 text-right">
                      <button className="text-emerald-900 hover:underline text-xs font-bold flex items-center justify-end gap-1 ml-auto">
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-8 flex flex-col md:flex-row md:items-center justify-between shadow-sm gap-6">
          <div className="flex items-center gap-6">
            <div className="w-16 h-16 bg-emerald-50 rounded-full flex items-center justify-center text-emerald-900">
              <span className="material-symbols-outlined text-4xl">analytics</span>
            </div>
            <div>
              <h4 className="font-bold text-slate-900">Resumo de Treinamento e Desenvolvimento</h4>
              <p className="text-sm text-slate-500">Métricas acumuladas do trimestre atual.</p>
            </div>
          </div>
          <div className="flex items-center gap-12">
            <div className="text-right">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Horas Treinadas</p>
              <p className="text-xl font-black text-slate-900">342h</p>
            </div>
            <div className="text-right">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Investimento</p>
              <p className="text-xl font-black text-emerald-900">R$ 12.450,00</p>
            </div>
          </div>
        </div>
      </div>
