...
import { crmService, type CRMLeadSource, type CRMLead } from '../../services/crm';
...
const EditLeadModal: React.FC<EditLeadModalProps> = ({ isOpen, onClose, onSuccess, lead }) => {
  ...
  useEffect(() => {
    if (isOpen && lead) {
      setFormData({
        ...
        estimated_potential: lead.estimated_potential?.toString() || '',
      ...
      const fetchContacts = async () => {
        try {
          const fetchedContacts = await crmService.getLeadContacts(lead.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Erro ao atualizar lead. Verifique os dados e tente novamente.');
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="p-8 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50">
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-slate-900 dark:text-white">Editar Lead</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400">Atualize os dados desta oportunidade e seus contatos.</p>
                  </div>
                </div>
                <button
                  onClick={onClose}
