        ...
        >
          ...
          <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-slate-900">Novo Tipo</h2>
                <p className="text-sm text-slate-500">Configuração de categoria de documento.</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Nome do Documento</label>
              <input
                type="text"
            ...
            {/* Description */}
            <div className="space-y-2">
              <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Descrição / Instruções</label>
              <textarea
                rows={2}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div 
                ...
                className={`p-4 rounded-2xl border-2 cursor-pointer transition-all ${
                  formData.requires_expiry ? 'border-emerald-900 bg-emerald-50' : 'border-slate-100 bg-slate-50'
                }`}
              >
                <div className="flex items-center justify-between mb-2">
                  <span className={`material-symbols-outlined ${formData.requires_expiry ? 'text-emerald-900' : 'text-slate-300'}`}>
                    event_repeat
                  </span>
                  <div className={`w-10 h-5 rounded-full transition-colors relative ${formData.requires_expiry ? 'bg-emerald-900' : 'bg-slate-200'}`}>
                    <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${formData.requires_expiry ? 'left-6' : 'left-1'}`} />
                  </div>
                </div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Possui Validade</p>
                <p className="text-xs font-bold text-slate-700">Controle de vencimento</p>
              </div>

              <div 
                onClick={() => setFormData({ ...formData, mandatory: !formData.mandatory })}
                className={`p-4 rounded-2xl border-2 cursor-pointer transition-all ${
                  formData.mandatory ? 'border-emerald-900 bg-emerald-50' : 'border-slate-100 bg-slate-50'
                }`}
              >
                <div className="flex items-center justify-between mb-2">
                  <span className={`material-symbols-outlined ${formData.mandatory ? 'text-emerald-900' : 'text-slate-300'}`}>
                    priority_high
                  </span>
                  <div className={`w-10 h-5 rounded-full transition-colors relative ${formData.mandatory ? 'bg-emerald-900' : 'bg-slate-200'}`}>
                    <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${formData.mandatory ? 'left-6' : 'left-1'}`} />
                  </div>
                </div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Obrigatório</p>
                <p className="text-xs font-bold text-slate-700">Exigido para todos</p>
              </div>
            </div>
            ...
            <AnimatePresence>
              {formData.requires_expiry && (
                ...
                >
                  <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Alerta de Vencimento (Dias antes)</label>
                  <input
                    type="number"
                    className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-emerald-900/20 focus:border-emerald-900 outline-none transition-all text-sm font-medium"
                  ...
                  />
                  <p className="text-[10px] text-slate-400 italic ml-1">Dica: O sistema avisará os gestores {formData.alert_days_before} dias antes da expiração.</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
